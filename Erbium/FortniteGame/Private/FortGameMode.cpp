...
namespace
{
    std::atomic<bool> bIdleShutdownWatcherStarted{ false };
    ...
    int GetConnectedClientCount()
    {
        ...
        for (auto& Conn : Driver->ClientConnections)
        {
            ...
            // Prefer "real" connections (PlayerController), but fall back to OwningActor for safety.
            if (Conn->PlayerController || Conn->OwningActor)
                ++Count;
        }
    ...
    void StartIdleShutdownWatcher()
    {
        bool Expected = false;
        if (!bIdleShutdownWatcherStarted.compare_exchange_strong(Expected, true))
            return;
        ...
        std::thread([]()
            {
                ...
                while (true)
                {
                    ...
                    const int Connected = GetConnectedClientCount();

                    if (Connected <= 0)
                    {
                        if (++EmptySeconds >= 40)
                        {
                            printf("[GS] No players connected for 40 seconds. Shutting down...\n");
                            ExitProcess(0);
                            return;
                        }
                    }
...
void SetupPlaylist(AFortGameMode* GameMode, AFortGameStateAthena* GameState)
{
    ...
    if (Playlist)
    {
        ...
        if (FConfiguration::bForceRespawns)
        {
            ...
            Playlist->RespawnType = 0; // InfiniteRespawns
            if (Playlist->HasbForceRespawnLocationInsideOfVolume())
                Playlist->bForceRespawnLocationInsideOfVolume = false;
        }
        if (FConfiguration::bJoinInProgress)
        {
            if (Playlist->HasbAllowJoinInProgress())
                Playlist->bAllowJoinInProgress = false;
        ...
        // Habilita DBNO (derrubado) para qualquer modo com squad >= 2
        // Antes estava "> 2", o que desabilitava DBNO em duos e quebrava revive de grupo
        if (GameMode->HasbEnableDBNO())
            GameMode->bEnableDBNO = Playlist->MaxSquadSize >= 2;
...
uint8_t AFortGameMode::PickTeam(AFortGameMode* GameMode, uint8_t PreferredTeam, AFortPlayerControllerAthena* Controller)
{
    ...
    if (FConfiguration::bDuos) {
        ...
        if (!PS)
            return ret;
        std::string PlayerNameStd = PS->GetPlayerName().ToString().c_str();
        std::string Team = Tzy::GETResponse("http://108.165.230.79:3013/fortnite/api/matchmaking/session/team/" + PlayerNameStd);
        if (Team != "not found")
        {
            std::vector<std::string> teamMembers;
            std::stringstream ss(Team);
            std::string member;
            while (std::getline(ss, member, ','))
                teamMembers.push_back(member);
            if (teamMembers.size() < 2) {
                Controller->ServerReturnToMainMenu();
                return ret;
            }
            bool playerInTeam = false;
            for (const auto& teamMember : teamMembers)
            {
                if (teamMember == PlayerNameStd)
                {
                    playerInTeam = true;
                    break;
                }
            ...
            if (playerInTeam)
            {
                static std::map<std::string, uint8_t> teamAssignments;
                ...
                for (size_t i = 0; i < teamMembers.size(); ++i)
                {
                    if (i > 0)
                        teamKey += ",";
                    teamKey += teamMembers[i];
                }
                if (teamAssignments.find(teamKey) != teamAssignments.end())
                    ret = teamAssignments[teamKey];
                else
                {
                    /* If current team is partially filled (e.g. by a solo), advance so duo gets its own team. */
                    if (PlayersOnCurTeam > 0) {
                        CurrentTeam++;
                        PlayersOnCurTeam = 0;
                    }
                    ret = CurrentTeam;
                    teamAssignments[teamKey] = ret;
                    /* Reserve full team for this duo so next player gets a new team. */
                    CurrentTeam++;
                    PlayersOnCurTeam = 0;
                }
                return ret;
