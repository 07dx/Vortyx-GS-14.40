...
bool UFortLootPackage::SpawnLootHook(ABuildingContainer* Container)
{
	...
	// Note: Accolades for search are handled in ServerOnAttemptInteract
	// This function is called from there, so we don't need to add accolades here
...
bool ServerOnAttemptInteract(ABuildingContainer* BuildingContainer, AFortPlayerPawnAthena* Pawn)
{
	printf("[Accolades] ServerOnAttemptInteract called\n");

	if (!BuildingContainer)
	{
		printf("[Accolades] BuildingContainer is null\n");
		return false;
	}

	if (BuildingContainer->bAlreadySearched)
	{
		printf("[Accolades] Container already searched\n");
		return true;
	}
	...
	printf("[Accolades] After SpawnLootHook, checking for accolades\n");

	// Give accolade for search
	if (Pawn && Pawn->Controller)
	{
		auto PlayerController = (AFortPlayerControllerAthena*)Pawn->Controller;
		if (PlayerController && PlayerController->XPComponent)
		{
			printf("[Accolades] PlayerController and XPComponent found\n");
			printf("[Accolades] Current PlayerAccolades count: %d\n", PlayerController->XPComponent->PlayerAccolades.Num());
			
			// Count total chests searched by summing all SearchChests accolades
			// This includes AccoladeId_007_SearchChests that the quest system gives
			int SearchCount = 0;
			for (auto& Accolade : PlayerController->XPComponent->PlayerAccolades)
			{
				if (Accolade.AccoladeDef)
				{
					FString AccoladeName = Accolade.AccoladeDef->Name.ToString();
					UEAllocatedWString AccoladeNameW = AccoladeName.ToWString();
					printf("[Accolades] Found accolade: %s (Count: %d)\n", AccoladeName.ToString().c_str(), Accolade.Count);
					// Count all SearchChests related accolades (007, 008, 009, 010, etc)
					if (AccoladeNameW.find(L"SearchChests") != std::wstring::npos || 
						AccoladeNameW.find(L"FirstSearchedChest") != std::wstring::npos)
					{
						// Sum all SearchChests accolades to get total count
						SearchCount += Accolade.Count;
						printf("[Accolades] Added to SearchCount: %d (new total: %d)\n", Accolade.Count, SearchCount);
					}
				}
			}
			
			// Increment search count (this will be the new total after this search)
			SearchCount++;
			
			printf("[Accolades] Total chests searched (after increment): %d\n", SearchCount);
			
			// Check if we should give an accolade based on the new count
			// Only give accolade at specific milestones (3, 7, 12)
			...
			{
				printf("[Accolades] Not a milestone (count: %d, need 3, 7, or 12)\n", SearchCount);
			}
		}
		else
		{
			printf("[Accolades] ERROR: PlayerController or XPComponent is null\n");
		}
	}
