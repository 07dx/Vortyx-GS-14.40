...
// Track chest searches ourselves (QuestManager accolades can have weird counts)
static std::unordered_map<AFortPlayerControllerAthena*, int32> GChestSearchCount;
...
void AFortPlayerControllerAthena::ServerAttemptAircraftJump_(UObject* Context, FFrame& Stack)
{
	...
	if (PlayerController && PlayerController->MyFortPawn && GPlayerControllers.Contains(PlayerController))
		PlayerController->MyFortPawn->K2_DestroyActor();
}
...
void AFortPlayerControllerAthena::ClientOnPawnDied(AFortPlayerControllerAthena* PlayerController, FFortPlayerDeathReport& DeathReport)
{
	...
	if (VersionInfo.FortniteVersion > 1.8 || VersionInfo.EngineVersion >= 4.19)
	{
		...
		{
			// Get kill count before incrementing
			int KillCount = KillerPlayerState->HasKillScore() ? KillerPlayerState->KillScore : KillerPlayerState->Kills;
			...
			// Increment kill count for accolade check
			KillCount++;
			
			// Give accolade for kill
			if (KillerPlayerController && KillerPlayerController->XPComponent)
			{
				auto AccoladeDef = GetDefFromEvent(EAccoladeEvent::Kill, KillCount);
				if (AccoladeDef)
				{
					GiveAccolade(KillerPlayerController, AccoladeDef);
				}
			}
			if (KillerPlayerState->HasTeamKillScore())
			{
				...
				std::string PUsername = KillerPlayerState->GetPlayerName().ToString().c_str();

				if (FConfiguration::bDuos) {
					for (auto& Actor : GameMode->AlivePlayers) {
						auto* PC = Actor->Cast<AFortPlayerControllerAthena>();
						if (!PC)
							continue;
						auto* PS = PC->PlayerState->Cast<AFortPlayerStateAthena>();
						if (!PS)
							continue;
						std::string PUsername_ = PS->GetPlayerName().ToString().c_str();
						if (PUsername_ != PUsername && PS->TeamIndex == KillerPlayerState->TeamIndex) {
							PS->TeamKillScore++;
							PS->OnRep_TeamKillScore();
							std::wstring usernameig_(PUsername_.begin(), PUsername_.end());
							Kdy::GiveVbucksForKill(usernameig_);
						}
					}
				}

				std::wstring usernameig(PUsername.begin(), PUsername.end());

				Kdy::GiveVbucksForKill(usernameig);
			}
			...
			if (KillerPlayerState && FConfiguration::bDuos) {
				std::string PUsername = KillerPlayerState->GetPlayerName().ToString().c_str();
				for (auto& Actor : GameMode->AlivePlayers) {
					auto* PC = Actor->Cast<AFortPlayerControllerAthena>();
					if (!PC)
		...
		{
			auto Playlist = VersionInfo.FortniteVersion >= 14.40 && GameMode->HasWarmupRequiredPlayerCount() ? (GameMode->GameState->HasCurrentPlaylistInfo() ? GameMode->GameState->CurrentPlaylistInfo.BasePlaylist : GameMode->GameState->CurrentPlaylistData) : nullptr;
...
void AFortPlayerControllerAthena::ServerClientIsReadyToRespawn(UObject* Context, FFrame& Stack)
{
	...
	auto Playlist = VersionInfo.FortniteVersion >= 3.5 && GameMode->HasWarmupRequiredPlayerCount() ? (GameMode->GameState->HasCurrentPlaylistInfo() ? GameMode->GameState->CurrentPlaylistInfo.BasePlaylist : GameMode->GameState->CurrentPlaylistData) : nullptr;
	const bool bRespawnsEnabled = Playlist && Playlist->RespawnType > 0;

	// Duo revive: only if respawns are enabled; if dead player's teammate is alive, set RespawnData from teammate's location
	if (bRespawnsEnabled && FConfiguration::bDuos && (!PlayerState->RespawnData.bRespawnDataAvailable || !PlayerState->RespawnData.bServerIsReady))
	{
		for (auto& Actor : GameMode->AlivePlayers)
		{
			auto* TeammatePC = Actor->Cast<AFortPlayerControllerAthena>();
			if (!TeammatePC || TeammatePC == PlayerController)
				continue;
			auto* TeammatePS = TeammatePC->PlayerState->Cast<AFortPlayerStateAthena>();
			if (!TeammatePS || TeammatePS->TeamIndex != PlayerState->TeamIndex)
				continue;
			if (!TeammatePC->Pawn)
				continue;
			auto* TeammatePawn = TeammatePC->Pawn->Cast<AFortPlayerPawnAthena>();
			if (!TeammatePawn)
				continue;
			// Teammate is alive - allow respawn near them
			PlayerState->RespawnData.RespawnLocation = TeammatePawn->K2_GetActorLocation() + FVector(100.f, 0.f, 0.f);
			PlayerState->RespawnData.RespawnRotation = TeammatePawn->K2_GetActorRotation();
	...
	if (bRespawnsEnabled && PlayerState->RespawnData.bRespawnDataAvailable && PlayerState->RespawnData.bServerIsReady)
	{
		...
		// Re-add to AlivePlayers when revived (duo or reboot)
		if (!GameMode->AlivePlayers.Contains(PlayerController))
		{
...
void AFortPlayerControllerAthena::ServerAttemptInteract_(UObject* Context, FFrame& Stack)
{
	...
	if (auto Container = ReceivingActor->Cast<ABuildingContainer>())
	{
		...
		// Give accolade for search - only if container wasn't already searched
		if (PlayerController && PlayerController->XPComponent && !Container->bAlreadySearched)
		{
			printf("[Accolades] ServerAttemptInteract_ - Container found, checking accolades\n");

			// Increment *our own* chest-search counter by 1 per container opened.
			// This avoids inflated counts coming from QuestManager accolades.
			int SearchCount = ++GChestSearchCount[PlayerController];
			printf("[Accolades] Total chests searched (tracked): %d\n", SearchCount);
			
			// Check if we should give an accolade based on the new count
			if (SearchCount == 3 || SearchCount == 7 || SearchCount == 12)
			{
				printf("[Accolades] Milestone reached: %d chests, giving accolade\n", SearchCount);
				
				// Verify PlayerController is still valid
				if (!PlayerController || !PlayerController->XPComponent)
				{
					printf("[Accolades] ERROR: PlayerController or XPComponent became invalid\n");
					return;
				}
				
				auto AccoladeDef = GetDefFromEvent(EAccoladeEvent::Search, SearchCount);
				if (AccoladeDef)
				{
					printf("[Accolades] Found accolade definition: %s\n", AccoladeDef->Name.ToString().c_str());
					
					// Double check before calling
					if (PlayerController && PlayerController->XPComponent && AccoladeDef)
					{
						GiveAccolade(PlayerController, AccoladeDef);
					}
					else
					{
						printf("[Accolades] ERROR: Parameters invalid before GiveAccolade call\n");
					}
				}
				else
				{
					printf("[Accolades] ERROR: Could not find accolade definition for count %d\n", SearchCount);
				}
			}
		}
	}
...
void AFortPlayerControllerAthena::ServerLoadingScreenDropped_(UObject* Context, FFrame& Stack)
{
	...
	Tzy::CheckPlayer(PlayerController);
