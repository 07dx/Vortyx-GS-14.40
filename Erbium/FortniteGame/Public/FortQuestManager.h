...
public:
    ...
    {
        float XpValue = XpRewardAmount.Evaluate();
        if (XpValue == 0)
            UDataTableFunctionLibrary::EvaluateCurveTableRow(XpRewardAmount.Curve.CurveTable, FName(L"Default_Medal"), XpRewardAmount.Value, nullptr, &XpValue, FString());
        return (int32)XpValue;
    }

    FText GetShortDescription() const
    {
        return HasDescription() ? Description : ItemDescription;
    }
};
