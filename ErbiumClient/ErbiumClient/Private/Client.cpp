...
void Client::Init()
{
	...
	if (VersionInfo.FortniteVersion >= 14 || FConfiguration::bForceRespawns)
	{
		...
		if (VersionInfo.FortniteVersion >= 14)
		{
			...
			FUIExtension AiKillsExtension{};
			AiKillsExtension.Slot = PrimarySlot; // ou 2 se for slot diferente
			AiKillsExtension.WidgetClass.ObjectID.AssetPathName = FName(L"/Game/Athena/HUD/AthenaAIKillsWidget.AthenaAIKillsWidget_C");

			ArenaExtensions.Add(ArenaUIExtension);
			ShowdownExtensions.Add(ShowdownUIExtension);
			ShowdownExtensions.Add(AiKillsExtension); // Adiciona AIKillsExtension às ShowdownExtensions
		}
